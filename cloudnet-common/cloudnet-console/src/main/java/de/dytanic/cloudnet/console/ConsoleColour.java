...
public enum ConsoleColour {
    ...
    ORANGE("orange", '8', Ansi.ansi().fg(Ansi.Color.RED).fg(Ansi.Color.YELLOW).bold().toString()),
    ...
    DARK_GRAY("dark_gray", 'b', Ansi.ansi().a(Ansi.Attribute.RESET).fg(Ansi.Color.BLACK).boldOff().toString());

    private final String name, ansiCode;

    private final char index;

    ConsoleColour(String name, char index, String ansiCode)
    {
        this.name = name;
        this.index = index;
    ...
    public static String toColouredString(char triggerChar, String text)
    {
        ...
        for(ConsoleColour consoleColour : values())
            text = text.replace(triggerChar + $.EMPTY_STRING + consoleColour.index, consoleColour.ansiCode);
